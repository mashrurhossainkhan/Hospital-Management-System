...
<?php
     if(isset($_SESSION["emailIsLoggedIn"])){
     	  $SavedEmail=$_SESSION["emailIsLoggedIn"];
...
$sql1 = "SELECT  `HospitalName` FROM `hospitalregistration` WHERE `Email`='$SavedEmail'";
  

 if($result1 = mysqli_query($conn, $sql1)){
    if(mysqli_num_rows($result1) > 0){
    	while($row = mysqli_fetch_array($result1)){
    			$HospitalAdmin=$row['HospitalName'];
    	}
    }
}
$sql = "SELECT * FROM `patientappointment` WHERE `HospitalName`='$HospitalAdmin'";
        ...
        while($row = mysqli_fetch_array($result)){
            ...
            echo "<div class='row'>";
                ...
                echo '<img class="img"; src="data:image/jpeg;base64,' . base64_encode( $row['Pic'] ) . '" />';
                    ...
                    echo "<p style='text-align:center;font-family: Comic Sans MS;'>Doctor's Name:".$row['AssignDoctor']."</p>";
					echo "<p style='text-align:center;font-family: Comic Sans MS;'>DEPARTMENT NAME:".$row['DepartmentName']."</p>";
                    echo "<p style='text-align:center;font-family: Comic Sans MS;'>ID:".$row['ID']."</p>";
