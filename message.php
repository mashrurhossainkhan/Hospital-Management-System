...
<?php
  ...
  if(isset($_GET['page'])){
        ...
        $Docname=$_SESSION["emailIsLoggedIn"];
  
$sql = "SELECT `PatientID` FROM `reports` WHERE `ReferredDoctor`='$Docname'";
 ...
 if($result = mysqli_query($conn, $sql)){
    if(mysqli_num_rows($result) > 0){
        while($row = mysqli_fetch_array($result)){
           $PassIDToClickDiv=$row['PatientID'];
            echo "<a href='ClickDivMessage.php?ID_Set=$PassIDToClickDiv'>";
            echo "<div >";
            	
                 echo "<p style= margin-left:20px>".$row['PatientID']."</p>";
